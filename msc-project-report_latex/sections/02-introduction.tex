...
A second failure mode, Failure B, sits on top of the first. Self-managing
system architectures typically close this kind of gap with an explicit
feedback path, most classically the monitor-analyse-plan-execute loop
used in autonomic computing (Kephart and Chess, 2003). Present day
cascade systems have no equivalent. They can run the same fixed
enrichment pipeline over every request, but no outcome ever travels
back to it. The system does not
...
an orchestration system that can see live data about its
...
\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item \textbf{Edge as an actor:} By providing an alternative approach
to edge devices with the Edge Context Protocol, devices will do more
than just return predictions. They will actively report their own
calibration status and, this way, close Failure A, a category of
error that these systems, such as BranchyNet, MSDNet, EdgeBERT, and
RouteLLM can't even detect.
\item \textbf{Medallion Pipeline with Dynamic Components:} A pipeline
that measures different things per request, per the situation and
input from the edge. Which positions in the state vector contain a
value changes, while the dimension of the vector itself remains fixed
at 13.
...
SLA remaining and predicted network. That granularity is missing from Splitwise (cluster/phase-level
...
life-cycle itself. Together with the dynamic
...
effect is small, which Section~\ref{conclusion} reports instead of claiming as a result.
