...
inference throughout. Two earlier runs are retained in the same
directory for traceability and are cited only in Section 5, where the
contrast itself is the finding, never as headline numbers here:
\textbf{evaluation/2026-08-20\_06-40-51.json} records the undertrained
policy at \textbf{fallback\_rate=1.000} in all three scenarios, and
...
specific, named file (\textbf{2026-08-23\_21-08-21.json}, referenced at
the top of this section) rather than a cherry-picked run among several.
Every earlier timestamped file in the same directory is either
superseded by it or cited explicitly, by name, at the point where its
contrast is itself the finding (the undertraining and reward-gradient
bugs, above). Each policy is run once per scenario (\textbf{n\_runs=1},
...
cost for its lower, more conservative escalation rate. This is reported
plainly rather than obscured: in the absence of infrastructure stress, a
continuous 13-dimensional policy surface does not have much to
differentiate on, and a simple threshold is a reasonable, cheap choice.
...
with SLA violations in the 64.5--66.0\% range. \textbf{run1\_full} holds
\textbf{zero} SLA violations, at roughly 19\% of the escalation rate and
19\% of the energy cost of \textbf{always\_cloud}, while matching its
accuracy to within one percentage point. The two edge-favouring
baselines (\textbf{always\_edge},
\textbf{routellm\_\-learned\_\-router}, which converged to a
near-always-edge policy) avoid SLA violations too, but at a real
accuracy cost that \textbf{run1\_full} does not pay. This is the
scenario the whole architecture is built to win, adaptive routing under
infrastructure stress, and the baselines it beats include both naive
...
\textbf{sla\_violation\_rate=0.000} against 0.655 for
\textbf{always\_cloud}, so the ceiling here is the 800ms RTT and not
the measure. What the scenario does
...
scenario's own extremity, but the policy neither collapses nor saturates
escalation, and \textbf{fallback\_rate} stays 0.000, meaning the
entropy/OOD safety net (Section 5) did not need to intervene even under
...
disabled)} tracks \textbf{run1\_full} closely on steady and bursty
traffic but degrades noticeably under degraded network (accuracy 0.760
vs.\ 0.815, the largest single gap in the table), consistent with the
backward loop's trust/error-rate signal mattering most when
the edge is under sustained stress, the condition it exists to detect.
...
7 both used the untrained \textbf{MiscalibrationClassifier}, which
falls through to its fixed \textbf{\_fallback\_heuristic()} threshold
(Section 5). After that ablation was complete, the classifier was
genuinely fitted on non-circular data (Section 5) and re-evaluated
against the identical \textbf{run\_ablation()} call, same sample size,
same scenarios.

\begin{longtable}{llrrrr}
\caption{run1\_full, untrained heuristic vs.\ trained classifier, at the report's own \texttt{max\_samples=1000}.}\label{tab:misc-classifier-comparison}\\
\toprule
...
The effect is consistent across every scenario, not an artefact of one
favourable run: escalation rate falls 82--85\% relative, p95 latency
falls 44--82\%, energy falls 22--53\%, and accuracy stays within
$\pm$0.015 either direction. \textbf{run4\_no\_reflect} shows the same
pattern in every scenario, which points at \texttt{operational\_state}
quality as the driver rather than the bidirectional loop. The
mechanism is a threshold-overlap effect, not a claim that the trained
...
policy was trained to treat that reveal as worth escalating on, so
suppressing the heuristic's over-firing directly suppresses
escalations. This is, in itself, dissertation-relevant evidence: it
shows the Silver enrichment layer's behaviour is sensitive to how
\texttt{operational\_state} is computed, and that a fixed threshold and
a modestly-accurate trained classifier (54.6\% balanced accuracy,
Section 5) can disagree often enough to move system-level metrics by
double digits.

The trained classifier is the default for every evaluation run past
this section (\textbf{environment.py}, opt-out via
\textbf{DEVMIND\_MISC\_CLASSIFIER\_PATH}). The seven-run ablation table
above and Run 7 (Section 7.4) are left as originally generated, under
the untrained heuristic, since regenerating every already-cited number
in this report under the new default was not attempted before
submission; this section exists precisely so that discrepancy is
documented rather than silently present. Full methodology for both
classifier attempts, the transfer-failure diagnosis for the first one,
and the raw before/after log this table is drawn from are in
